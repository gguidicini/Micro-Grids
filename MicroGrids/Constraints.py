...
def Total_Fuel_Cost(model,s,g):
    ...
    for y in range(1, model.Years +1):
        Num = sum(model.Total_Generator_Energy[s,y,g,t]*model.Generator_Marginal_Cost[g] for t in model.periods)
...
def Scenario_Variable_Cost(model, s):
    ...
    return model.Total_Scenario_Variable_Cost[s] == model.Operation_Maintenance_Cost + model.Battery_Reposition_Cost[s] + model.Scenario_Lost_Load_Cost[s] + Fuel_Cost


def Total_Variable_Cost_Obj(model):
    
    return (sum(model.Total_Scenario_Variable_Cost[s]*model.Scenario_Weight[s] for s in model.scenarios))
